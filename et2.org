...
* Net 网络交互相关：【服务端+客户端】只是稍微改装成事件系统而已
...
- 【会话框】：哈哈哈，这是会话框两端，哪一端的场景呢？分不清。。。去找出来！客户端？网关服？
#+BEGIN_SRC csharp
// 消息分发组件
...
public enum MailboxType {
   ...
   #+END_SRC
** ActorMessageSenderComponent: 管理类组件
   #+BEGIN_SRC java
[ComponentOf(typeof(Scene))]
public class ActorMessageSenderComponent: Entity, IAwake, IDestroy {
   ...
   #+END_SRC
** ActorMessageSenderComponentSystem: 多带个Timer
- 这个类我好像认真读过，大概是小伙伴搬家超时，什么之类的，改天再读一遍
   #+BEGIN_SRC java
...
public abstract class ConfigSingleton<T>: ProtoObject, ISingleton where T: ConfigSingleton<T>, new() {
   ...
   #+END_SRC
** StartProcessConfigCategory : ConfigSingleton<StartProcessConfigCategory>, IMerge: 吃饱了撑得吗？
