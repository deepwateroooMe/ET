...
namespace ET { // 暂时把这里仍然放在双端
    // [ComponentOf(typeof(UI))]  // 这里说，如果这个组件也作为UI 的子控件，同样要标记；而标记两个以上的，就不用再注明类型
    [ComponentOf]  // 这里说，如果这个组件也作为UI 的子控件，同样要标记；而标记两个以上的，就不用再注明类型，先去掉一会儿
