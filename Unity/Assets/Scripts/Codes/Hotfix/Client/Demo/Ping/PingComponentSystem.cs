...
namespace ET.Client {
    ...
    [ObjectSystem]
    public class PingComponentAwakeSystem: AwakeSystem<PingComponent> { // Awake
        protected override void Awake(PingComponent self) { // Awake
            PingAsync(self).Coroutine();
        }

        // 忘记了: 为什么要PING来着? 服务器在等待多久之后需要PING? 怎么PING才是效率最高不浪费性能的?
