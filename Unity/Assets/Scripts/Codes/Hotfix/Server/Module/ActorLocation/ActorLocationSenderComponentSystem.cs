...
// Awake Destroy    
    ...
    public static class ActorLocationSenderComponentSystem {
        ...
        public static async ETTask<IActorResponse> Call(this ActorLocationSenderComponent self, long entityId, IActorRequest iActorRequest) {
            ...
            // 先序列化好
            int rpcId = ActorMessageSenderComponent.Instance.GetRpcId(); // 这里拿到的是某(区?)里的某个劳工标识
            iActorRequest.RpcId = rpcId;
            
            long actorLocationSenderInstanceId = actorLocationSender.InstanceId;
            using (await CoroutineLockComponent.Instance.Wait(CoroutineLockType.ActorLocationSender, entityId)) { // 给来把锁 ?
// 过滤掉过程中的报错                
