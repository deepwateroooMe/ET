...
namespace ET.Server {
	...
	{
		// protected override async void Run(Room room, Actor_PlayerEnterRoom_Req message, Action<Actor_PlayerEnterRoom_Ack> reply)
		protected override void Run(Room room, Actor_PlayerEnterRoom_Req message, Actor_PlayerEnterRoom_Ack response)
		{
			...
			try
			{
				...
				{  // 这里大致把这部分的逻辑看一下，要去弄好吃了。。。。。【这里没看完，下次接着看。爱表哥，爱生活！！！】
				   // 玩家重连
				...
				//reply(response); // 这里暂时就把它理解为：框架的底层封装会帮助自动回复消息，帮把回复的步骤封装起来了
			}
			catch (Exception e)
			{
				//ReplyError(response, e, reply); // 这里先跳过。。。
			}
		}
	}
}