    ...
    public static class UILobbyComponentSystem {
        ...
        public class UILobbyComponentAwakeSystem: AwakeSystem<UILobbyComponent> {
            protected override void Awake(UILobbyComponent self) { 
                ...
                InitLobbyComponent(self); // 补充：获取玩家数据【因为已经登录成功了，才能进到这里厅里的】
            }
        }
        private static async void InitLobbyComponent(this UILobbyComponent self) {
            ...
            G2C_GetUserInfo_Ack g2C_GetUserInfo_Ack = await self.ClientScene().GetComponent<SessionComponent>().Session.Call(c2G_GetUserInfo_Req)
        ...
        public static async ETTask matchRoom(this UILobbyComponent self) { // 通过网关服中转，请求匹配服为给匹配一个房间四人桌
            try {
                ...
                G2C_StartMatch_Ack g2C_StartMatch_Ack = await self.ClientScene().GetComponent<SessionComponent>().Session.Call(c2G_StartMatch_Req)
                    as G2C_StartMatch_Ack; 
