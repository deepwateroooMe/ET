...
namespace ET {
    ...
    public abstract class AService: IDisposable { // 基类：现在重点找：网络服务，读到消息后的大致过程
        public int Id { get; set; }
        public ServiceType ServiceType { get; protected set; } // <<<<<<<<<<<<<<<<<<<< 
        private (object Message, MemoryStream MemoryStream) lastMessageInfo; // 用来：缓存上一个发送的消息，这样广播消息的时候省掉多次序列化
        // 缓存上一个发送的消息，这样广播消息的时候省掉多次序列化。
        // 这样有个另外的问题,客户端就不能保存发送的消息来减少gc

		// 【下面：一堆杂乱】直接把这个相关底层，再看一遍，先前写什么一堆。。
        // 【上面一句，应该可以理解为，使用场景如客户端需要重复发某条消息时（如索要位置信息，但小伙伴搬家搬狠久发一次还要不到，客户端连发 10 次索要位置信息，每次消息内容完全相同）
        // 【上面：这里，自己的例子仍不确切，自己举了一个某种需求消息层面的重复消息，客户端是单线程还是多线程逻辑——单线程，同一个客户端同一时间点是否可能多条消息——不可以，消息发送队列在哪里？】
        // 【上面：但内存流上未必最后一条消息。需要是客户端发送消息队列里、或内存流层面上与最后一条消息相同】】，每次都得生成新的，这种场景下，客户端就会产生大量GC. 这些牛角尖：每个客户端缓存自己的最后一条消息，及对应然序列化后的内存流，管它是不是内存流上的最后一个呢？！
        ...
        protected MemoryStream GetMemoryStream(object message) { // 拿消息的内存流：是（广播消息的时候）的一个使用需求
            ...
            this.lastMessageInfo = (message, stream); // 缓存上一个发送的消息：缓存的是，上一个消息的、序列化后的内存流，省得再发重复消息时，还得再【序列化】一遍，序列化慢呀
            return stream;
        }
