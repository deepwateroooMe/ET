...
namespace ET {
    public class WService: AService {
        ...
        private readonly Dictionary<long, WChannel> channels = new Dictionary<long, WChannel>(); // 管理这类型的服务下，所有客户端的字典 
        public ThreadSynchronizationContext ThreadSynchronizationContext; // 同步
        public WService(IEnumerable<string> prefixs) {
            this.ThreadSynchronizationContext = new ThreadSynchronizationContext();
            this.httpListener = new HttpListener();
            StartAccept(prefixs).Coroutine(); // 协程：专职，只负责接收各客户端的连接请求。管理入字典
        }
        public WService() {
            this.ThreadSynchronizationContext = new ThreadSynchronizationContext();
        }
        ...
        private async ETTask StartAccept(IEnumerable<string> prefixs) {
            try {
                ...
                httpListener.Start();
                while (true) { // 【专职、专线程？】：只负责接收，各种客户端的连接请求
                    try {
