...
namespace ET {
    public struct KcpWaitPacket {
        public long ActorId; // 它封装了： actorId. 应该是跨进程必备
...
// 发送请求连接消息
        ...
        private void KcpSend(KcpWaitPacket kcpWaitPacket) {
            ...
            switch (this.Service.ServiceType) {
            case ServiceType.Inner: { // 【内网消息】：写 ActorId
                memoryStream.GetBuffer().WriteTo(0, kcpWaitPacket.ActorId);
                break;
            }
            case ServiceType.Outer: { // 外网不需要发送actorId，跳过。（这是因为，外网消息，消息体自带 actorId. 这里消息头再写，是没有必要的！）
                // 外网不需要发送actorId，跳过。【前源者注】这里把概念上统一：【内网消息】与【外网消息】
            ...
            this.Service.AddToUpdate(0, this.Id); // 每桢更新，或是最懒到点儿才更新，服务端维持一套自更新机制（原理没去细看）
        }
        public void Send(long actorId, MemoryStream stream) {
            ...
            if (!this.IsConnected) {
                this.sendBuffer.Enqueue(kcpWaitPacket); // 【断联了缓存】：体现出的好处是，网络状况不好的情况下，因为缓存机制，连上后再发，不太容易丢包，但有延迟
                return;
            }
            if (this.kcp == IntPtr.Zero) 
