...
namespace ET {
    // 看下面定义的类型：Kcp 服务与路由器交互的消息确认等，感觉这个模块的基本原理都还不懂。先把这个底层类大致扫一遍
            ...
            while (socket != null && this.socket.Available > 0) {
				...
				try {
					...
					switch (flag) {
						...
						case KcpProtocalType.SYN: { // accept: 【网络模块】相对底层的知识点
							// 长度!=5，不是SYN消息
							if (messageLength < 9) 
								break;
							string realAddress = null;
							...
							remoteConn = BitConverter.ToUInt32(this.cache, 1); // 从固定类型的消息里，解析出来的，这些参数
							localConn = BitConverter.ToUInt32(this.cache, 5);
							this.waitAcceptChannels.TryGetValue(remoteConn, out kChannel);
							if (kChannel == null) { // 还没真正建立起【会话框】
