...
// 等待异步任务的创建完成? 是这样的吗？不像，更像下面的：更像是，等待异步任务的结果写好。写好的结果，这里是一把拿到了索要资源的【保障共享资源安全】的、不站队排队的锁CoroutineLock
            // 【现在快想通了吧】：每当回收一把刚使用完共享资源的锁（CoroutineLock-Dispose()），回收的过程与逻辑就是，通知队列中的下一个排队的锁：hi, 我用完了，你等到了你的顺序，你接下来就可以享用使用共享资源了！！
