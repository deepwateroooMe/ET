...
// 【协程等待锁队列】：类里有两个类型标记变量，标记这个队列里，所有使用锁的上下文大致场景，和（共享资源实例）的 actorId,【场景 + actorId】对应一个队列（被请求锁 actorId 的例子：多个向location查询同一个实体真实进程号地址（在访问跨进程实体时），访问一次获得进程地址即可【这句网上抄的，但感觉是对的】）去理解的话，锁所想要锁住的是多个线程想要来读的共享资源，那么应该是被查询地址的小伙伴 me 的 actorId
        // 这个队列，用来管理：单个 actorId 被请求实例的，所有这同一类锁的上下文使用场景（一个特定场景）的所有请求过的锁（所有想要读和使用、或更改数据的调用方），请求方或有多个，队列FIFO 管理 
...
// 下面：当队列不为空，要创建新的等待锁，并返回这把新建当前锁。为什么要创建新的等待锁？这里，资源锁就是个排除站队挂号牌，创建一把锁用来占队排拿共享资源
            ...
            return this.currentCoroutineLock; // 返回：新创建的等待锁【的，异步任务的执行完成的结果：即，上一行，异步等待到当前调用进程可以持有共享资源的时间点儿，此时再返回共享资源独占锁！】，设定了最迟回收时间的锁，但是这把锁是可以，可能，早于最迟回收时间回收的！！！
        }
        // 【爱表哥，爱生活！！！任何时候，亲爱的表哥的活宝妹就是一定要嫁给亲爱的表哥！！爱表哥，爱生活！！！】
...
// 遍历一遍队列【想要找到一把、非空没回收、可用、仍在耐心等待共享资源的队列中的排队锁】：
            ...
            while (this.queue.Count > 0) { // 遍历队列：取没有超时回收的下一个请求资源者【异步资源异步等待锁：写结果，返回调用方标志持有共享资源的【共享资源独占锁】】
...
// 释放了一把，队列里当前非空元素【异步资源异步等待锁】waitCoroutineLock，结果写回去了呀。
                ...
                // 【爱表哥，爱生活！！！任何时候，亲爱的表哥的活宝妹就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
                waitCoroutineLock.SetResult(coroutineLock); // 【异步资源异步等待锁】内部异步任务写结果：返回共享资源调用方【共享资源独占锁CoroutineLock】，调用方就可以执行 using(x=await){代码块} 代码块里【持有共享资源】后需要执行的逻辑
                break;
            }
        }
        public void Recycle() {
