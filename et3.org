...
#+END_SRC
** ConfigAttribute: 抓这个出去，想去理解，当扫标签属性时，扫到的应该是形如阅读过的【Invoke(TimerInvokeType.ActorMessegaeSenderChecker)】之类的AttributeTargets.Class 特定类的类型？
- 结合接下来 ConfigLoader.cs 中的源码，多想几遍或查一下，确认自己理解正确了。
- 框架里有狠多【Config】标签。这里我在看【Invoke|(TimerInvokeType.ActorMessegaeSenderChecker)】相关时，好像把Invoke 与Config 弄混了。它们是两个不同的标签。
- 还需要把【Config】标签独立出来，再读一下
#+BEGIN_SRC csharp
...
- 为什么这个类，会是写了两遍呢？有什么不同？要想一想。总结不该是，通过写，和一再阅读复习和思考，把原本不曾摆放一起的内容摆一起，帮助自己想透彻背后的原理吗？
