...
\label{sec-3-17}
\begin{itemize}
\item 这个以前没有细看。但是现在，框架里有狠多细节的地方，有计时器来触发必要的超时等。所以现在把这一块儿捡一下
...
\begin{itemize}
\item 又想到一点，ActorMessageSender, 既可以是发送消息者发送消息的发送器，也可以是，错误码返回消息的发送器。那么就是说，ActorMessageSenderComponent 的循环逻辑某处，是可以发返回消息的。
\item 发送消息超时异常，不走发返回消息路径，而是直接由ETTask 抛异常，不需要发返回消息。Run() 方法被其它情境下调用，才会发返回消息，系统的后半部分，有发送消息的逻辑，今天不看那块儿了，改天再看那部分。
\end{itemize}
