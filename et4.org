...
- 所以这个类就是定义，异步任务协程中的一个特殊状态：异步任务结束了，但是还没能写、返回结果时的IAsyncStateMachine|IAwater 状态
- 因为它是协程异步状态机中的一个相对特殊状态，本质上是异步状态机中的一个状态，也提供了必要的API, 比较写结果。
   #+BEGIN_SRC csharp
...
- *【IMHandler】* ：在 ET7 的框架里，Handle() 方法的定义，主要是Actor 消息机制大概又重构得更为自动化一点儿，就是说大致是说，当有分门别类的ActorMessageHandler 标签系实体类，大概ET7 框架里只需要调用接口的申明方法就可以了？总之，就是Handle() Run() 两大类方法的方法定义发生了变化，但现在还没弄明白本质，为什么在ET7 框架里可以简化，实现简化的原理？
  - AMRpcHandler 的实体实现类，我可能还需要再多找几个出来看下
  - 不知道我刚才笔记本上提交的内容在哪里。今天下午弄这些IMHandler 以及两个抽象实现类，和它们服务端的消息处理器类，编译氏错误一堆，主要是自己这里还有点儿不太懂。
- *【内存消息等网络相关：请求消息的发送方法等】*
  - NetInnerComponent NetOuterComponent: 全部移除了？
