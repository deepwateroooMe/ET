...
- *【HandCardSprite.cs】* ：这个客户端文件里存在一堆关于Unity 引用的错误。把这个有着巨多错误的类重新添加到了框架里。现在着眼着这些错误（加了这个文件，错误又多了二三十个！！！）。
  ...
  - 必须认识到， *【参考项目Game.cs】* 客户端类里，存在UnityEngineer 的诸多引用，所以HandCardSprite.cs 可以通过Game.EventSystem 等拿到引用。但ET7 重构得没有边际。必须自己去看明白。
...
public static class Game {
    ...
    #+END_SRC
  - 这里客户端，关于Unity 的引用，可能还没有连好  
- 现框架里不存在的，需要整合进来的模块版块：
  - 这里列一下剩下的模块，方便将来修改：
#+BEGIN_SRC text
System.Net.IPEndPoint to long
ComponentFactory ComponentWithId
DBProxyComponent
EnterMapHelper
GamerComponent
InnerConfig
...
- 上午就是看那几个框架里还不熟悉的模块，能看懂哪些，就先看先事理哪些。
